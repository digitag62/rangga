...
import { deleteCat } from "@/lib/cat/actions";

export const DeleteDialogCat = ({ data, setIsOpen, setSelectedCat }: { data: CatProps; setIsOpen: (val: boolean) => void; setSelectedCat: (Cat: CatProps | null) => void }) => {
	const [open, setOpen] = useState(false);
	const [load, setLoad] = useState(false);

	const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
		e.preventDefault();
		const res = await deleteCat(data.id, data.userId ?? "");
