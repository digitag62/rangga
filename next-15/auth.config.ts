...
export const authConfig = {
  ...
  // This is not required, but by adding signIn: '/sign-in' into our pages option,
  ...
  callbacks: {
    ...
    authorized({ auth, request: { nextUrl } }) {
      ...
      if (isOnDashboard) {
        if (isLoggedIn) return true; // If logged in redirect to dashboard
        return false; // Redirect unauthenticated users to login page
      } else if (isLoggedIn) {
        return Response.redirect(new URL("/dashboard", nextUrl)); // If not in dashboard but logged in, redirect to dashboard
      }
