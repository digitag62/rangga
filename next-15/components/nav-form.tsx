...
export function NavForm({ navGroup, role }: navFormProps) {
	...
	async function onSubmit(values: z.infer<typeof formSchema>) {
		...
		setIsLoading(true);
		const url = `${navGroup.filter((nav) => nav.id === values.group)[0].url}/${slugify(values.title)}`;
			...
			<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
				<div className="grid gap-4">
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								...
								<FormControl>
									<MultiSelect options={roleOptions} onValueChange={field.onChange} defaultValue={field.value} placeholder="Select role" variant="inverted" animation={0} maxCount={3} />
								</FormControl>
								<FormMessage />
							</FormItem>
						)}
					...
					<div className="flex justify-center items-center gap-2 mt-2">
						<Link href="/dashboard/settings/nav" className={cn(buttonVariants({ variant: "outline" }), "w-full")}>
							Cancel
						</Link>
